...
const createOrGetOneToOneChat = async (req, res) => {
  ...
  if (existingChat.length) {
    return res
      .status(200)
      .json(new ApiResponse(200, existingChat[0], 'Chat fetched Successfully'));
  }
...
const removeParticipantFromGroupChat = async (req, res) => {
  ...
  // event sent to other participants in the group to notify them
  existingChat.participants.forEach((participantId) => {
    emitSocketEvent(
      req,
      participantId.toString(),
...
const joinGroupChat = async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(201, existingChat, 'Successfully Joined the group'));
};
